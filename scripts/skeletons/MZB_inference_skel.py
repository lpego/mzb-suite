...
dirs = find_checkpoints(Path(f"{prefix}mzb-skels"), version="bprys4hk", log="last") # mit_b1 tversky loss longT
...
data_dir = Path("/data/shared/mzb-classification/data/raw_learning_sets_duben/insects/")
...
def paint_image(image: torch.Tensor, masks: torch.Tensor, color: List[float]) -> torch.Tensor:
    """
    ...
    for mask in masks: 
        ...
        rgb_body[mask > 0.75] = torch.Tensor([color[c][0] * mask[mask > 0.75], 
                                            color[c][1] * mask[mask > 0.75], 
                                            color[c][2] * mask[mask > 0.75]]).permute((1,0))
        c += 1
...
for i, ind in enumerate(sortind):
    ...
    refined_skel = np.concatenate([(skeletonize(a)>0).astype(float)*mask for a in p[0:1,...]>0.75])
    # refined_skel[1,...] *= 2
...
if 0: 
    ...
    for i, ti in enumerate(im_fi[:]):
        ...
        refined_skel = Image.fromarray((255*np.transpose(refined_skel,(1,2,0))).astype(np.uint8))
        ...
        if MASK: 
            ...
            mask = np.array(transforms.Resize(
                    (o_size[1], o_size[0]),
                    interpolation=transforms.InterpolationMode.BILINEAR,
                )(mask))
            refined_skel = [(skeletonize(a)>0).astype(float)*mask for a in refined_skel[0:2,...]>50]
        else: 
            # Refine the predicted skeleton image
            refined_skel = [(skeletonize(a)>0).astype(float) for a in refined_skel[0:2,...]>50]
        
        preds_size.append(pd.DataFrame({"clip_name": "_".join(ti.name.split(".")[0].split("_")[:-1]), 
    ...
    preds_size = preds_size.merge(annot, left_index=True, right_index=True, how="inner")
    ...
    print(f"MSE NN to manual: \t body: {np.sqrt(np.mean(np.sum(err['body_true_to_nn']**2)))}")
    print(f"\t \t  \t head: {np.sqrt(np.mean(np.sum(err['head_true_to_nn']**2)))}")

    print(f"MSE auto to manual: \t body skel: {np.sqrt(np.mean(np.sum(err['body_true_to_sk']**2)))}")
    print(f"\t \t  \t body elli: {np.sqrt(np.mean(np.sum(err['body_true_to_el']**2)))}")
    print(f"\t \t  \t head elli: {np.sqrt(np.mean(np.sum(err['head_true_to_el']**2)))}")
