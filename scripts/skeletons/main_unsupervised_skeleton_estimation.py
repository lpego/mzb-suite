...
if args.list_of_files is not None:
    ...
    predictions = pd.read_csv(args.list_of_files).set_index("file").sort_values("file")
    exclude = predictions[
...
err_filenames = sorted(
    list(
        Path(
            f"{prefix}/data/learning_sets/project_portable_flume/curated_learning_sets/errors"
        ).glob("*.png")
    )
)
exclude += [
