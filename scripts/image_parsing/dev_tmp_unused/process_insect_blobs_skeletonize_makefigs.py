...
out_fold_examples = Path("../outputs_examples/")
out_fold_examples.mkdir(exist_ok=True, parents=True)

MASKID = [23, 24, 25]
submask = [mask_proc[a] for a in MASKID]
for fi, fo in enumerate(submask):
    print(
    ...
    # mm_ = erosion(mask_, selem=disk(7))
    mm_ = mask_[..., np.newaxis]
    ...
    if 1:
        rgb_cutout = (cv2.imread(str(fo)[:-8] + "rgb.png")[:, :, [2, 1, 0]]).astype(
            np.uint8
        )  #
        ...
        x1 = x0 + np.cos(props.orientation) * 0.5 * props.minor_axis_length
        y1 = y0 - np.sin(props.orientation) * 0.5 * props.minor_axis_length
        x2 = x0 - np.sin(props.orientation) * 0.5 * props.major_axis_length
        y2 = y0 - np.cos(props.orientation) * 0.5 * props.major_axis_length

        f, a = plt.subplots(1, 1, figsize=(20, 4))
        a.imshow(rgb_cutout)
        a.axis("off")

        # a[0].plot(rrs, ccs, color="blue")
        # a[0].title.set_text("rgb and encl ell")
        f, a = plt.subplots(1, 1, figsize=(20, 4))
        a.imshow(masked_im)
        a.axis("off")

        # a[1].title.set_text("raw / ell clip ell")
        sub_col = ["white", "lightgray", "red", "green"]
        subc = ListedColormap(sub_col)

        long_sk = binary_dilation(skel_long, disk(5)) != 0
        short_sk = binary_dilation(linef_, disk(5)) != 0
        skels = 2 * long_sk + short_sk - (long_sk & short_sk)

        f, a = plt.subplots(1, 1, figsize=(20, 4))
        # a.imshow(fil.skeleton)  # (linef_ & masf_)
        # rgb_cutout
        a.imshow(mask_ + skels)#, cmap=subc[])
        a.axis("off")
        ...
        f, a = plt.subplots(1, 1, figsize=(20, 4))
        # a.imshow(fil.skeleton)  # (linef_ & masf_)
        skels = binary_dilation(skels, disk(11))
        skels = skels.astype(float)
        skels[skels==0] = np.nan
        a.imshow(rgb_cutout)
        a.imshow(skels,cmap=subc)
        a.axis("off")
        # a.contour((linef_ & mask).astype(np.uint8), c="pink")

        f, a = plt.subplots(1, 1, figsize=(20, 4))
        a.imshow(rgb_cutout)
        a.plot(rrs, ccs, color="blue")
        a.axis("off")

        # a.title.set_text("rgb and encl ell")
        # a[2].title.set_text("longest skeleton")
        # a[3].imshow(mask_skel)
        # a[3].title.set_text("full skeleton from [1]")
        # a[4].imshow(distance)
        # a[4].title.set_text("distance transform")
        # a[5].imshow(masf_)
        # a[5].plot((x0, x1), (y0, y1), "-r", linewidth=2.5)
        # a[5].plot((x0, x2), (y0, y2), "-r", linewidth=2.5)
        # a[5].title.set_text("encl ell and min/max radii")
        # plt.show()
